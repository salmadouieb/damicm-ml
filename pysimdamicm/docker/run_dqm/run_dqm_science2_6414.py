...
REPROCESSED_FITS_FILES='/home/wuser/workdir/dqm_manual/science-run2/reprocessed_file_list_{}.log'.format(CCDNAME)

# raw data location at cca.in2p3.fr for SCIENCE RUN 2  -- CCD ID 6414
DATA_DIR='/data/fits/2022-05-10-science-ccd{}'.format(CCDNAME)

# pattern file name for the images in SCIENCE RUN 2 -- CCD ID 6414
...
def run_dqm(next_image_id, logfile):
    """RUN THE SCRIPT dqm_workflow_LBC_science_run2_ccd6414.sh which runs dqmSKImg for a given image number
    """
    
    run_number = get_run_number_id(next_image_id)

    bash_command_line = [ './dqm_workflow_LBC_science_run2_ccd6414.sh',
            run_number,
            str(next_image_id),
            str(next_image_id)
            ]
...
def do_print():
    with open('kkita.txt','a+') as f:
        f.writelines('HOOOOOOOLA:'+time.ctime()+'\n')
    return
